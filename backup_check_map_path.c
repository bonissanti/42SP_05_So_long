...
int flood_fill(t_game *game, int x, int y, char target, char replacement, int *count, int **visited)
{
	...
	original = game->map.map_matriz[x][y];
	...
	game->map.map_matriz[x][y] = original;
	return (result);
}
...
char **copy_map_matriz(char **original, int rows, int columns)
{
	char **copy;
	int x;
	int y;
...
void print_map(t_game game)
{
	...
	while (row < game.map.rows)
	{
		...
		{
			printf("%c", game.map.map_matriz[row][col]);
			col++;
		}
		printf("\n");
		row++;
...
int main(void)
{
	...
	while (game.map.rows--)
		free(game.map.map_matriz[game.map.rows]);
	free(game.map.map_matriz);
